...
The project must be impressive as a technical artifact while remaining free to run. Requiring external APIs, cloud accounts, or private credentials would make the evaluation experience fragile and harder to reproduce.
...
- Technical evaluators can run the project with `npm install` and `npm run dev`.
