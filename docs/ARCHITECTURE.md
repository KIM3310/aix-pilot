...
`src/data/businessModel.ts` and `src/lib/valueModel.ts` add a service launch layer above the product. The Value Readiness model calculates monthly recovered workflow time, weekly human review load, recommended service scope, and operating transition signal. It also models stakeholder personas, behavioral economics, culture-aware rollout patterns, resource-focused expansion paths, and measurable validation experiments.
