...
- `pnpm run lint`로 TypeScript와 React 코드 경계를 확인합니다.
- `pnpm run test`로 Golden Evaluation, Service Trust Model, 서비스 범위, 도입 모델, 개인정보 마스킹 리포트 계약을 실행합니다.
- `pnpm run build`로 정적 배포 산출물을 생성합니다.
- 데스크톱/모바일 브라우저에서 콘솔 에러, 오버플로우, 주요 CTA 동작을 확인합니다.
